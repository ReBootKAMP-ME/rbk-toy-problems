...
Tree.prototype.map = function (callback) {
  ...
  var traverse = function(oldTree, newTree){
    if(oldTree.children.length === 0){
      return 
    }

    for(var i = 0; i < oldTree.children.length; i++){
      newTree.children[i] = new Tree(callback(oldTree.children[i].value))
      traverse(oldTree.children[i], newTree.children[i])
    }
  }

  traverse(this, newRoot)
...
Tree.prototype.mapInPlace = function (callback) {
  ...
  var traverse = function(tree){
    ...
    for(var i = 0; i < tree.children.length; i++){
      tree.children[i].value = callback(tree.children[i].value)
