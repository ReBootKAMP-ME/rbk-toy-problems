...
var coinCombination = function(totalAmount){
	var generalCirculation = [1,2,5,10,20,50,100,200];
	var totalNumberOfCombinations = 0;
	
	var getAllTryOuts =  function(number){
		if(number === totalAmount){
			console.log('Add 1');
			totalNumberOfCombinations++;
			return;
		}
		for(var i = 0 ; i < generalCirculation.length; i++){
			if(number+=generalCirculation[i] <= totalAmount){
			  getAllTryOuts(number+=generalCirculation[i]);
			}
		}
	}
