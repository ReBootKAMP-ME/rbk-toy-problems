...
Tree.prototype.map = function (callback) {
    newTree = new Tree(callback(this.value))
    var solution = function (newTree, oldTree){
      if(oldTree.children && oldTree.children.length>0){
        for (var i = 0; i < oldTree.children.length; i++){
          newTree.addChild(callback(oldTree.children[i].value))
          solution (newTree.children[i], oldTree.children[i]);
        }
      } else {
        return;
      }
    }
    solution(newTree, this)
    console.log(newTree)
};
...
Tree.prototype.mapInPlace = function (callback) {
  ...
  var solution = function (tree){
    tree.value = callback(tree.value)
    if (tree.children && tree.children.length > 0){
      for ( var i = 0; i < tree.children.length; i++ ){
        solution (tree.children[i])
      }
    } else {
      return;
    }
