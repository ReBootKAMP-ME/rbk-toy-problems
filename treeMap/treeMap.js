...
Tree.prototype.map = function (callback) {
  // return a new tree with the same structure as `this`, with values generated by the callback
   var helper = function (callback, rTree, newTree) {
      if(newTree === undefined){
        newTree = new Tree(callback(rTree.value))
      }else{
        newTree.addChild(new Tree(callback(rTree.value)))
      }
      if(rTree.children.length > 0){
        for (var i = 0; i < rTree.children.length; i++) {
            helper(callback, rTree.children[i], newTree)
        }
      }
      return newTree;
    }
  
  return helper(callback, this);
};
